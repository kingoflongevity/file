...
func main() {
	...
	a, err = astilectron.New(nil, astilectron.Options{
		AppName: "Remote File Manager",
	})
	if err != nil {
		log.Fatalf("Failed to initialize astilectron: %v", err)
	}
	defer a.Close()

	// 设置应用事件处理器
	a.On("app.ready", func(e astilectron.Event) (deleteListener bool) {
		// 创建窗口
		w, err = a.NewWindow(fmt.Sprintf("http://localhost:%d", cfg.Port), &astilectron.WindowOptions{
			Center: astikit.BoolPtr(true),
			Height: astikit.IntPtr(800),
			Width:  astikit.IntPtr(1200),
			Title:  astikit.StrPtr("远程连接文件管理"),
		})
		if err != nil {
	...
	// 设置窗口关闭事件处理器
	a.On("app.window.all.closed", func(e astilectron.Event) (deleteListener bool) {
		// 关闭应用
		if err := a.Quit(); err != nil {
			log.Fatalf("Failed to quit app: %v", err)
		}
		return false
	})
